...
class RoundRobin:
    ...
    def select_server(self):
        self.i += 1
        return self.servers[self.i%len(self.servers)]
...
class LeastConnections:
    ...
    def select_server(self):
        index_min = self.connections.index(min(self.connections))
        self.connections[index_min] += 1
        return self.servers[index_min]

    def update(self, *arg):
        if arg[1] == "delete":
            index = self.servers.index(arg[0])
...
class LeastResponseTime:
    def __init__(self, servers):
        self.servers = servers
        self.times = dict.fromkeys(servers,0)
        self.t = 0
    def select_server(self):
        min_t = min(self.times, key=self.times.get)
        return min_t

    def update(self, *arg):
        if arg[1] == "add":
            now = datetime.now()
            self.t = datetime.timestamp(now)

        if arg[1] == "delete":
            now = datetime.now()
            diff_temp = datetime.timestamp(now) - self.t
            self.times[arg[0]] += diff_temp
            self.t = 0
...
def accept(sock, mask):
    ...
    policy.update(policy.select_server(), "add")

def read(conn,mask):
    data = conn.recv(4096)
    if len(data) == 0: # No messages in socket, we can close down the socket
        mapper.delete(conn)
        policy.update(policy.select_server(), "delete")
    else:
